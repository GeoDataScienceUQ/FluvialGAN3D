...
class D3Model(torch.nn.Module):
    ...
    def forward(self, noise, data, mode, num):
        ...
        input_semantics, real_image, real_bottom = self.preprocess_input(data,num)

        if mode == 'generator':
            g_loss, generated = self.compute_generator_loss(
                noise, input_semantics, real_image, real_bottom,num)
            ...
            with torch.no_grad():
                fake_image,fake_cube,fake_semantics = self.generate_fake(noise, input_semantics,num)
            return fake_cube
    ...
    def preprocess_input(self, data,num):
        ...
        if num == 1:
            ...
            return data['label'], data['image'], data['label']
        elif num==3:
            data['up3'] = data['up3'].cuda()
            data['up2'] = data['up2'].cuda()
            return data['label'], data['up3'], self.onehot(data['up2'])
        elif num==5:
            data['up5'] = data['up5'].cuda()
            data['up4'] = data['up4'].cuda()
            return data['label'], data['up5'], self.onehot(data['up4'])
        elif num==7:
            data['up7'] = data['up7'].cuda()
            data['up6'] = data['up6'].cuda()
            return data['label'], data['up7'], self.onehot(data['up6'])

    def compute_generator_loss(self, noise, input_semantics, real_image, real_bottom,num):
        G_losses = {}

        fake_image,fake_cube,fake_bottom = self.generate_fake(noise, input_semantics,num)
    ...
    def compute_discriminator_loss(self, noise, input_semantics, real_image, real_bottom,num, use_gp=True):
        D_losses = {}
        with torch.no_grad():
            fake_image,fake_cube,fake_bottom = self.generate_fake(noise, input_semantics,num)
    ...
    def generate_fake(self, noise, input_semantics, num):
        if num==1:
            fake_image = self.netG(noise, input_semantics)
        else:
            fake_image = self.netG(noise, input_semantics)
            for i in range(num-1):
                input_semantics = fake_image.detach()
                fake_image = self.netG(noise,input_semantics)
