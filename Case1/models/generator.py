...
class SPADEGenerator(Module):
    def __init__(self, args):
        ...
        self.head_0 = SPADEResBlk(args, 16 * self.nf, 16 * self.nf)

        self.G_middle_0 = SPADEResBlk(args, 16 * self.nf, 16 * self.nf)

        self.up_0 = SPADEResBlk(args, 16 * self.nf, 8 * self.nf)
        self.up_1 = SPADEResBlk(args, 8 * self.nf, 4 * self.nf)
        self.up_2 = SPADEResBlk(args, 4 * self.nf, 2 * self.nf)
        self.up_3 = SPADEResBlk(args, 2 * self.nf, self.nf)
        self.conv_img = nn.Conv2d(self.nf, 7 * args.img_nc, 3, padding=1)
